...
export default function HomePage() {
    ...
    const fetchData = async () => {
        try {
            const repoData = await fetch('http://0.0.0.0:3000/arsync_repos', {
                method: 'GET',
                headers: {
    ...
    useEffect(() => {
        ...
        if (codeParam && (localStorage.getItem("accessToken") === null)) {
            async function getAccessToken() {
                await fetch('http://0.0.0.0:3000/getAccessToken?code=' + codeParam, {
                    method: 'GET',
                }).then((response) => {
                    return response.json()
                }).then((data) => {
                    ...
                    if (data.access_token) {
                        ...
                        window.location.href = "http://0.0.0.0:5173/dashboard";
                    }
                })
            }
            getAccessToken()
